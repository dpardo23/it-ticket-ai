...
export function TerminalPanel({ logs, isProcessing, terminalRef }: TerminalPanelProps) {
    ...
    const TerminalContent = ({ isModal = false }: { isModal?: boolean }) => (
        <>
            <div className="h-[40px] bg-muted/30 border-b border-border/60 px-4 flex items-center justify-between transition-colors duration-500">
                ...
                {!isModal ? (
                    <Button
                        variant="ghost"
                        size="icon"
                        className="h-6 w-6 text-muted-foreground hover:bg-muted hover:text-foreground rounded-md transition-colors"
                    ...
                    <Button
                        variant="ghost"
                        size="icon"
                        className="h-6 w-6 text-muted-foreground hover:bg-red-500/10 hover:text-red-500 dark:hover:text-red-400 rounded-md transition-colors"
            ...
            <div
                ...
                className={`flex-1 overflow-y-auto font-mono leading-relaxed whitespace-pre-wrap break-words bg-background transition-colors duration-500 ${isModal ? 'p-6 text-[13px]' : 'p-4 text-[11px]'}`}
            >
                {logs.map((log, i) => (
                    <div
                        key={i}
    ...
    return (
        <>
            ...
            {isExpanded && (
                <div
                    className="fixed inset-0 z-[100] flex items-center justify-center bg-background/80 backdrop-blur-sm p-6 animate-in fade-in duration-300"
                    onClick={() => setIsExpanded(false)}
                >
                    <div
                        className="w-full max-w-5xl h-[75vh] shadow-2xl animate-in zoom-in-95 duration-300 rounded-xl overflow-hidden border border-border/60 flex flex-col bg-card"
